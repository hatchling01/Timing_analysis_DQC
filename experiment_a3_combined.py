...
class MemoryPool:
    """Simple shared memory pool with blocking semantics."""
    ...
    def alloc(self):
        if self.used >= self.capacity:
            raise RuntimeError("alloc called when pool is full")
        self.used += 1

    def free(self):
        if self.used <= 0:
            raise RuntimeError("free called when pool is empty")
        self.used -= 1

class EntanglementServerCombined(Protocol):
    """
    Variant 3: Combined bottlenecks

    Two-stage pipeline at shared switch S:

      Stage A (parallel, limited by memory K):
        - If memory has space, request starts "generation attempts" (stochastic time).
        - Multiple Stage A processes can run concurrently, up to K in-flight.

      Stage B (serialized, single BSM device):
        - After Stage A completes, request waits for BSM (capacity=1).
        - BSM adds fixed processing time, then completes the request and frees memory.

    This makes BOTH constraints visible:
      - Memory pressure builds up when BSM becomes the bottleneck.
      - BSM queue builds up when memory allows many in-flight generations.
    """
    ...
    def __init__(
        ...
        self.q: Deque[EPRRequest] = deque()          # incoming requests
        self.ready_bsm: Deque[EPRRequest] = deque() # finished gen, waiting for BSM
        ...
        self._bsm_busy = False

    def submit(self, req: EPRRequest):
        self.q.append(req)

    def enqueue_ready_for_bsm(self, req: EPRRequest):
        self.ready_bsm.append(req)

    def _sample_gen_time(self) -> int:
    ...
    def _start_gen(self, req: EPRRequest, gen_time: int):
        sub = _GenCompletionCombined(
            f"gen_{req.tenant}_{req.req_id}",
            self,
            req,
            gen_time
        )
    ...
    def _maybe_start_bsm(self):
        """
        Start BSM processing if:
          - BSM is idle
          - there is a request waiting for BSM
        """
        if self._bsm_busy or not self.ready_bsm:
            return False

        self._bsm_busy = True
        self._current_bsm_req = self.ready_bsm.popleft()
        return True

    def run(self):
        while True:
            # ---- Stage A admission: start as many gens as memory allows ----
            started_any = False
            while self.q and self.mem.has_free():
                req = self.q.popleft()
                self.mem.alloc()            # reserve memory immediately
                started_any = True
                gen_time = self._sample_gen_time()
                self._start_gen(req, gen_time)

            # ---- Stage B: serialized BSM processing ----
            if self._maybe_start_bsm():
                # occupy the BSM
                yield self.await_timer(self.bsm_time_ns)

                # complete request, free memory
                t_done = ns.sim_time()
                req = self._current_bsm_req
                req.on_done(req.tenant, req.req_id, req.t_req, t_done)
                self.mem.free()
                self._bsm_busy = False

                # loop continues; may immediately start another BSM or gen

            # small yield to advance time and let subprotocols enqueue
            yield self.await_timer(1)


class _GenCompletionCombined(Protocol):
    """
    Represents the "generation attempt" stage that can run in parallel,
    limited by memory capacity. When done, it hands the request to the
    parent's BSM queue (single-capacity stage).
    """
    def __init__(self, name: str, parent: "EntanglementServerCombined",
                 req: EPRRequest, gen_time: int):
        super().__init__(name=name)
        self.parent = parent
        self.req = req
    ...
    def run(self):
        yield self.await_timer(self.gen_time)
        # generation finished; now must pass through single BSM stage
        self.parent.enqueue_ready_for_bsm(self.req)
...
def run_once(
    ...
    srv = EntanglementServerCombined(
    "S_combined",
    mem_capacity=mem_capacity,
    p_success=server_p_success,
    bsm_time_ns=bsm_time_ns
    )
    print(f"SERVER CONFIG: K={mem_capacity}, bsm_time_ns={bsm_time_ns}")
    srv.start()
    print("SERVER CLASS:", type(srv).__name__)
...
if __name__ == "__main__":
    ...
    log_off = run_once(attacker_period_ns=attacker_period, victim_period_ns=victim_period, seed=7, victim_on=False, mem_capacity=mem_K, bsm_time_ns=bsm_time)

    log_on  = run_once(attacker_period_ns=attacker_period, victim_period_ns=victim_period, seed=8, victim_on=True, mem_capacity=mem_K, bsm_time_ns=bsm_time)
