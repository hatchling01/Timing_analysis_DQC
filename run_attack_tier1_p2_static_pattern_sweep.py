#!/usr/bin/env python3
"""
run_attack_tier1_p2_static_pattern_sweep.py

Static deployment + Tier-1 attacker + overlap placement (P2 first overlap case)
with a PROBE-PATTERN sweep.

Fixed:
- overlap placement P2:
    victim   on module_0, module_1, module_2
    attacker on module_2, module_3
- dummy anchor qubit on module_4 for validator
- fixed rate      : R1_dense
- fixed timescale : short = P20
...
def build_probe1(total_rounds: int):
    """
    Probe 1:
    Heavier direct probe, two cross-module interactions per round.
    """
    qc = QuantumCircuit(4)
...
def build_probe2(total_rounds: int):
    """
    Probe 2:
    Intermediate probe, one cross-module CX plus more local mixing.
    """
    qc = QuantumCircuit(4)
    for _ in range(total_rounds):
        qc.h(0)
...
def build_probe3(total_rounds: int):
    """
    Probe 3:
    Best light structured probe from prior study.
    """
    qc = QuantumCircuit(4)
    for _ in range(total_rounds):
        qc.h(0)
...
def build_probe_circuit(probe_name: str, total_rounds: int):
    if probe_name == "Probe1":
        return build_probe1(total_rounds)
    if probe_name == "Probe2":
        return build_probe2(total_rounds)
    if probe_name == "Probe3":
        return build_probe3(total_rounds)
    raise ValueError(f"Unknown probe name: {probe_name}")


def generate_probe_trace(probe_name: str, total_rounds: int):
    q2m = attacker_probe_qubit_map()
    qc = build_probe_circuit(probe_name, total_rounds)
...
def run_one_experiment(victim_qasm: str, probe_cfg: dict):
    ...
    attacker_trace, attacker_q2m, _ = generate_probe_trace(
...
def plot_request_level(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Overlap P2 Pattern Sweep: Request-Level ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"overlap_p2_pattern_{victim_tag}_request_level.png", dpi=300)
    plt.show()
...
def plot_job_makespan(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Overlap P2 Pattern Sweep: Job Makespan ({victim_tag})")
    plt.tight_layout()
    plt.savefig(f"overlap_p2_pattern_{victim_tag}_job_makespan.png", dpi=300)
    plt.show()
...
def plot_job_counts(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Count")
    plt.title(f"Overlap P2 Pattern Sweep: Job Counts ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"overlap_p2_pattern_{victim_tag}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        for probe_cfg in PROBE_SWEEP:
            print(f"Running overlap P2 pattern sweep on {victim_tag}: {probe_cfg['name']}")
            result = run_one_experiment(victim_qasm, probe_cfg)
            print(result)
            results.append(result)

        out_json = f"overlap_p2_pattern_{victim_tag}_results.json"
