#!/usr/bin/env python3
"""
Experiment A3 (updated): combined memory + serialized BSM contention
with explicit local modular topology.

Topology:
    A_mod ----\
               \
                >---- S_mod  (shared memory-limited generation + serialized BSM)
               /
    V_mod ----/
...
- Stage A: requests use shared memory and undergo generation in parallel
- Stage B: completed generations wait for one serialized BSM resource
"""
...
class LocalModularTopology:
    """
    Explicit local modular topology for A3.
...
# ============================================================
# Shared resources at S_mod
# ============================================================

class MemoryPool:
    """
    Shared finite-capacity pool.
    In A3, one slot is allocated when Stage A starts and freed only
    after Stage B (serialized BSM) completes.
    """
...
class EntanglementServerCombined(Protocol):
    ...
    - Stage A: memory-limited parallel generation
    - Stage B: serialized BSM

    Total latency =
        queueing before Stage A
      + generation attempts + heralding
      + queueing before Stage B
      + BSM time
    """
    ...
    def __init__(
        ...
        self.arrival_q: Deque[EPRRequest] = deque()
        self.ready_bsm: Deque[EPRRequest] = deque()
    ...
    def _maybe_start_bsm(self):
        if self._bsm_busy:
            return
        if not self.ready_bsm:
            return

        req = self.ready_bsm.popleft()
        self._bsm_busy = True

        sub = _BsmSubprotocol(
            name=f"bsm_{req.tenant}_{req.req_id}",
            server=self,
            req=req,
            bsm_time_ns=self.bsm_time_ns,
        )
        sub.start()
    ...
    def run(self):
        while True:
            ...
            # Stage A admissions: start parallel generation while memory is available
            while self.arrival_q and self.mem.available():
                req = self.arrival_q.popleft()
                self.mem.allocate()
                generation_time = self._sample_generation_time()

                sub = _GenerationStageSubprotocol(
            ...
            # Try to start BSM if possible
            if (not self._bsm_busy) and self.ready_bsm:
                self._maybe_start_bsm()
                started_any = True
...
class _GenerationStageSubprotocol(Protocol):
    """
    Stage A: generation attempts + heralding.
    Memory remains occupied after this stage until BSM completes.
    """
    def __init__(
        self,
        name: str,
        server: EntanglementServerCombined,
        req: EPRRequest,
    ...
    def run(self):
        ...
        # Stage A completed; request now waits for serialized BSM
        self.server.ready_bsm.append(self.req)
        self.server._maybe_start_bsm()


class _BsmSubprotocol(Protocol):
    """
    Stage B: serialized BSM processing.
    Completion frees memory.
    """
    def __init__(
        self,
        name: str,
    ...
    def run(self):
        ...
        # Completion of whole request
        self.server.mem.free()
        self.server._bsm_busy = False

        t_done = ns.sim_time()
        self.req.on_done(self.req.tenant, self.req.req_id, self.req.t_req, t_done)

        # Immediately try to start next BSM request, if any
        self.server._maybe_start_bsm()
...
def run_once(
    ...
    """
    Run one A3 simulation on the explicit local modular topology.
    ...
    A3 semantics:
    - Stage A: memory-limited parallel generation at S_mod
    - Stage B: serialized BSM at S_mod
    """
    sim_init(seed)
