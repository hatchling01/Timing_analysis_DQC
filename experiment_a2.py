...
class EntanglementServerMemoryOnly(Protocol):
    ...
    Memory-only bottleneck:
      - No single-capacity BSM queue.
      - Entanglement generation can proceed whenever there is free memory.
      - If memory full: request waits (blocking).
      - Service time still stochastic (attempts), but not serialized by a 1-capacity device.
    """
    ...
    def run(self):
        """
        Event loop:
          - If there is a pending request and memory is free, start it immediately.
          - Otherwise wait.
        Implementation detail: for memory-only, we allow multiple in-flight generations,
        ...
        while True:
            ...
            while self.q and self.mem.has_free():
                ...
                # spawn a sub-process for completion
                self._start_gen(req, gen_time)

            # If nothing could start, yield a bit
            if not started_any:
                yield self.await_timer(1)
            else:
                # if we started something, yield to let those timers progress
                yield self.await_timer(1)
    ...
    def _start_gen(self, req: EPRRequest, gen_time: int):
        ...
        sub.start()
class _GenCompletion(Protocol):
    def __init__(self, name: str, parent: "EntanglementServerMemoryOnly", req: EPRRequest, gen_time: int):
        super().__init__(name=name)
        self.parent = parent
        self.req = req
...
def run_once(
    ...
    srv = EntanglementServerMemoryOnly("S_mem", mem_capacity=mem_capacity, p_success=server_p_success)
    srv.start()
    print("SERVER_CLASS", type(srv).__name__)
