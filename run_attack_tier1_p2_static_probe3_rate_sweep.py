...
- keep relative-duration framework
- sweep probe RATE under overlap
...
# Keep the same relative windows for comparison
...
# Rate sweep for Probe 3 under overlap
# total_rounds is computed from victim cross-module ops and duration fraction
# num_probe_rounds controls how many of those rounds contain the cross-module CX
RATE_SWEEP = [
    {"name": "R1_dense",       "density": 1.00},
    {"name": "R2_medium",      "density": 0.50},
    {"name": "R3_light",       "density": 0.25},
    {"name": "R4_sparse",      "density": 0.125},
    {"name": "R5_very_sparse", "density": 0.0625},
]
...
def pick_uniform_probe_rounds(total_rounds: int, num_probe_rounds: int):
    """
    Uniform spacing of probe rounds within total_rounds.
    """
    if total_rounds <= 0:
        return set()
    if num_probe_rounds <= 0:
        return set()
    if num_probe_rounds >= total_rounds:
        return set(range(total_rounds))
    if num_probe_rounds == 1:
        return {total_rounds // 2}

    step = (total_rounds - 1) / (num_probe_rounds - 1)
    rounds = sorted(set(int(round(i * step)) for i in range(num_probe_rounds)))
    return set(rounds)


def build_probe3_with_rate(total_rounds: int, density: float):
    """
    Probe 3 with variable rate:
    - every round has the same local scaffold
    - only selected rounds include the cross-module CX
    """
    num_probe_rounds = max(1, math.ceil(total_rounds * density)) if total_rounds > 0 else 0
    probe_rounds = pick_uniform_probe_rounds(total_rounds, num_probe_rounds)
    ...
    return qc, num_probe_rounds


def generate_probe3_rate_trace(total_rounds: int, density: float):
    q2m = attacker_probe_qubit_map()
    qc, num_probe_rounds = build_probe3_with_rate(total_rounds, density)
    trace = trace_from_qiskit_circuit(qc, q2m)
    return trace, q2m, qc.num_qubits, num_probe_rounds
...
def combine_victim_attacker_mappings(v_q2m, v_num_qubits, a_q2m):
    ...
    # dummy anchor qubit so all five modules appear
...
def run_one_experiment(victim_qasm: str, duration_cfg: dict, rate_cfg: dict):
    ...
    attacker_trace, attacker_q2m, _, num_probe_rounds = generate_probe3_rate_trace(
        total_rounds=total_rounds,
        density=rate_cfg["density"],
    )
...
def plot_request_level(results, victim_tag, duration_name):
    ...
    plt.ylabel("Time (ns)")
    plt.title(f"Overlap P2 Rate Sweep: Request-Level ({victim_tag}, {duration_name})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"overlap_p2_ratesweep_{victim_tag}_{duration_name}_request_level.png", dpi=300)
    plt.show()


def plot_job_makespan(results, victim_tag, duration_name):
    labels = [r["rate_name"] for r in results]
    ...
    plt.xticks(x, labels, rotation=20)
    plt.ylabel("Time (ns)")
    plt.title(f"Overlap P2 Rate Sweep: Job Makespan ({victim_tag}, {duration_name})")
    plt.tight_layout()
    plt.savefig(f"overlap_p2_ratesweep_{victim_tag}_{duration_name}_job_makespan.png", dpi=300)
    plt.show()


def plot_job_counts(results, victim_tag, duration_name):
    labels = [r["rate_name"] for r in results]
    ...
    plt.xticks(x, labels, rotation=20)
    plt.ylabel("Count")
    plt.title(f"Overlap P2 Rate Sweep: Job Counts ({victim_tag}, {duration_name})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"overlap_p2_ratesweep_{victim_tag}_{duration_name}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        for duration_cfg in RELATIVE_DURATION_SWEEP:
            ...
            for rate_cfg in RATE_SWEEP:
                print(f"Running overlap P2 rate sweep on {victim_tag}: {duration_cfg['name']} / {rate_cfg['name']}")
                result = run_one_experiment(victim_qasm, duration_cfg, rate_cfg)
                print(result)
                duration_results.append(result)
                all_results.append(result)

            plot_request_level(duration_results, victim_tag, duration_cfg["name"])
            plot_job_makespan(duration_results, victim_tag, duration_cfg["name"])
            plot_job_counts(duration_results, victim_tag, duration_cfg["name"])

        out_json = f"overlap_p2_ratesweep_{victim_tag}_results.json"
