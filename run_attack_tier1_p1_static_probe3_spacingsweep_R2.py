#!/usr/bin/env python3
"""
run_attack_tier1_p1_static_probe3_spacing_sweep_R2.py

Probe 3 spacing sweep at fixed medium probe rate (R2-like).

Same setup as R1 file, but fixed to about 15 cross-module probes over 30 rounds.
"""
...
# Fixed medium rate: about 15 probe injections over 30 rounds
SPACING_SWEEP_R2 = [
    {"name": "S1_uniform_medium", "pattern": "uniform", "total_rounds": 30, "num_probes": 15},
    {"name": "S2_pairs_medium", "pattern": "pairs", "total_rounds": 30, "num_probes": 15},
    {"name": "S3_clustered_medium", "pattern": "clustered", "total_rounds": 30, "num_probes": 15},
    {"name": "S4_alt_medium", "pattern": "alternating", "total_rounds": 30, "num_probes": 15},
]
...
def pick_probe_rounds(total_rounds: int, num_probes: int, pattern: str):
    ...
    if pattern == "pairs":
        rounds = []
        base_gap = max(3, total_rounds // max(1, num_probes // 2))
        r = 0
    ...
    if pattern == "alternating":
        ...
        r = 0
        short_gap = 1
        long_gap = max(4, total_rounds // max(2, num_probes // 2))
        toggle = True
        while len(rounds) < num_probes and r < total_rounds:
            rounds.append(r)
...
def run_one_experiment(victim_qasm: str, spacing_cfg: dict):
    ...
    return {
        ...
        "probe": "probe_3_spacing_sweep_R2",
...
def plot_request_level(results, victim_tag):
    ...
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R2 Spacing Sweep: Request-Level ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_spacing_R2_{victim_tag}_request_level.png", dpi=300)
    plt.show()
...
def plot_job_makespan(results, victim_tag):
    ...
    plt.xticks(x, labels, rotation=30, ha="right")
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R2 Spacing Sweep: Job Makespan ({victim_tag})")
    plt.tight_layout()
    plt.savefig(f"probe3_spacing_R2_{victim_tag}_job_makespan.png", dpi=300)
    plt.show()
...
def plot_job_counts(results, victim_tag):
    ...
    plt.xticks(x, labels, rotation=30, ha="right")
    plt.ylabel("Count")
    plt.title(f"Probe 3 R2 Spacing Sweep: Job Counts ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_spacing_R2_{victim_tag}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        results = []
        for spacing_cfg in SPACING_SWEEP_R2:
            print(f"Running R2 spacing sweep on {victim_tag}: {spacing_cfg['name']}")
            results.append(run_one_experiment(victim_qasm, spacing_cfg))

        out_json = f"probe3_spacing_R2_{victim_tag}_results.json"
