...
def build_sparse_bursty_schedule(victim_trace, attacker_trace, period, burst_size):
    """
    A5-like schedule with bursts:
    - victim runs continuously
...
def plot_request_level(results, probe_name):
    ...
    plt.ylabel("Time (ns)")
    plt.title(f"Sparse Bursty Sweep: Request-Level ({probe_name})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"burst_sweep_{probe_name}_request_level.png", dpi=300)
    plt.show()


def plot_job_level(results, probe_name):
    labels = [r["burst_name"] for r in results]
    ...
    plt.xticks(x, labels, rotation=30, ha="right")
    plt.ylabel("Time (ns)")
    plt.title(f"Sparse Bursty Sweep: Job Makespan ({probe_name})")
    plt.tight_layout()
    plt.savefig(f"burst_sweep_{probe_name}_job_makespan.png", dpi=300)
    plt.show()
...
def main():
    # change this line for each victim workload
    ...
    for probe_name in ATTACKER_PROBES:
        ...
        out_json = f"burst_sweep_{probe_name}_results.json"
        with open(out_json, "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2)
        print(f"Saved {out_json}")

        plot_request_level(results, probe_name)
        plot_job_level(results, probe_name)
