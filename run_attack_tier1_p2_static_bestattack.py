#!/usr/bin/env python3
"""
run_attack_tier1_p2_static_bestattack.py

Static deployment + Tier-1 attacker + first OVERLAP placement case.

Placement used here (P2-style first overlap case):
- victim on  module_0, module_1, module_2
- attacker on module_2, module_3
- one dummy anchor qubit on module_4 so the five-module validator passes

So attacker overlaps with victim on module_2.
"""
...
def combine_victim_attacker_mappings(v_q2m, v_num_qubits, a_q2m):
    ...
    # add one dummy anchor qubit on module_4 so validator sees all five modules
...
def run_one_experiment(victim_qasm: str, duration_cfg: dict):
    ...
    return {
        ...
        "probe": "probe3_R1_uniform_static_overlap_p2",
...
def plot_request_level(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Static Overlap P2: Request-Level ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"static_overlap_p2_{victim_tag}_request_level.png", dpi=300)
    plt.show()
...
def plot_job_makespan(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Static Overlap P2: Job Makespan ({victim_tag})")
    plt.tight_layout()
    plt.savefig(f"static_overlap_p2_{victim_tag}_job_makespan.png", dpi=300)
    plt.show()
...
def plot_job_counts(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Count")
    plt.title(f"Static Overlap P2: Job Counts ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"static_overlap_p2_{victim_tag}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        for duration_cfg in RELATIVE_DURATION_SWEEP:
            print(f"Running static overlap P2 on {victim_tag}: {duration_cfg['name']}")
            result = run_one_experiment(victim_qasm, duration_cfg)
            print(result)
            results.append(result)

        out_json = f"static_overlap_p2_{victim_tag}_results.json"
