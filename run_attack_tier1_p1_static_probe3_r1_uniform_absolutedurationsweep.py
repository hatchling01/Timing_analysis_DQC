#!/usr/bin/env python3
"""
run_attack_tier1_p1_static_probe3_duration_sweep_absolute.py
...
Sweep:
- absolute total probe rounds: 10, 20, 50, 100, 200
"""
...
ABSOLUTE_DURATION_SWEEP = [
    {"name": "D10", "total_rounds": 10},
    {"name": "D20", "total_rounds": 20},
    {"name": "D50", "total_rounds": 50},
    {"name": "D100", "total_rounds": 100},
    {"name": "D200", "total_rounds": 200},
]
...
def run_one_experiment(victim_qasm: str, duration_cfg: dict):
    ...
    attacker_trace, attacker_q2m, _ = generate_probe3_duration_trace(duration_cfg["total_rounds"])
    ...
    return {
        ...
        "probe": "probe3_R1_uniform_duration_absolute",
        "duration_name": duration_cfg["name"],
        "total_rounds": duration_cfg["total_rounds"],
...
def plot_request_level(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R1 Uniform: Absolute Duration Sweep ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_absdur_{victim_tag}_request_level.png", dpi=300)
    plt.show()
...
def plot_job_makespan(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R1 Uniform: Absolute Duration Sweep Makespan ({victim_tag})")
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_absdur_{victim_tag}_job_makespan.png", dpi=300)
    plt.show()
...
def plot_job_counts(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Count")
    plt.title(f"Probe 3 R1 Uniform: Absolute Duration Sweep Counts ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_absdur_{victim_tag}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        results = []
        for duration_cfg in ABSOLUTE_DURATION_SWEEP:
            print(f"Running absolute duration sweep on {victim_tag}: {duration_cfg['name']}")
            results.append(run_one_experiment(victim_qasm, duration_cfg))

        out_json = f"probe3_R1_uniform_absdur_{victim_tag}_results.json"
