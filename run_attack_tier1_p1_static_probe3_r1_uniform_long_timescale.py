#!/usr/bin/env python3
"""
Long-timescale victim sensing:
uses relative-duration windows: P50, P100
...
def run_one_experiment(victim_qasm: str, duration_cfg: dict):
    ...
    return {
        ...
        "probe": "probe3_R1_uniform_long_timescale",
...
def plot_request_level(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R1 Uniform Long Timescale ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_long_{victim_tag}_request_level.png", dpi=300)
    plt.show()
...
def plot_job_makespan(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Time (ns)")
    plt.title(f"Probe 3 R1 Uniform Long Timescale Makespan ({victim_tag})")
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_long_{victim_tag}_job_makespan.png", dpi=300)
    plt.show()
...
def plot_job_counts(results, victim_tag):
    ...
    plt.xticks(x, labels)
    plt.ylabel("Count")
    plt.title(f"Probe 3 R1 Uniform Long Timescale Counts ({victim_tag})")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"probe3_R1_uniform_long_{victim_tag}_job_counts.png", dpi=300)
    plt.show()
...
def main():
    for victim_qasm in VICTIM_QASMS:
        ...
        for duration_cfg in RELATIVE_DURATION_SWEEP:
            print(f"Running long-timescale sweep on {victim_tag}: {duration_cfg['name']}")
            results.append(run_one_experiment(victim_qasm, duration_cfg))

        out_json = f"probe3_R1_uniform_long_{victim_tag}_results.json"
